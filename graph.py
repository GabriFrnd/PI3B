...
class GrafoDenso(Grafo):
    ...
    def centralidade_intermediacao(self):
        ...
        for s in range(self.n):
            ...
            sigma = dict.fromkeys(self.vertices, 0)
            sigma[self.vertices[s]] = 1
            dist = dict.fromkeys(self.vertices, -1)
            dist[self.vertices[s]] = 0
            ...
            while stack:
                ...
                for v in pred[w]:
                    ...
                    delta[v] += delta_v
                if w != self.vertices[s]:
                    betweenness[w] += delta[w]
        return betweenness

    def pagerank(self, d=0.85, max_iter=100, tol=1e-6):
        N = self.n
        pr = {v: 1/N for v in self.vertices}
        for _ in range(max_iter):
            ...
            for v in self.vertices:
                ...
                novo_pr[v]=rank
            if all(abs(novo_pr[v]-pr[v])<tol for v in self.vertices):
                break
            pr = novo_pr
        return pr
